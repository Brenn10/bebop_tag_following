...
bebopTagFollowing::bebopTagFollowing(ros::NodeHandle& nh) {
  ...
  dispatch = nh.subscribe("/birdseye/dispatch",1,&bebopTagFollowing::dispatchDrone,this);
  ...
  if(dispatch_state>=3){
    ...
    switch(dispatch_state){
      ...
      case turning://in turning state
        ...
        cmdVel.publish(cmdT);
        if(abs(ang_dist(state.yaw,disp_angle_))<5)
        {
